public class PalindromePartitionOptimized {
    ...
    private static int minPalPartition(String str, int[][] memTable, int i, int j) {
        ...
        for(int k=i;k<j;k++) {
            int left = memTable[i][k] != -1 ? memTable[i][k] : minPalPartition(str, memTable, i, k);
            int right = memTable[k+1][j] != -1 ? memTable[k+1][j] : minPalPartition(str, memTable, k+1, j);
